...
def autolog(
    ...
    **TensorFlow Core**
    ...
    class EagerTrain(PatchFunction):#Added
        def __init__(self):
            self.log_dir = None
        ...
        def _patch_implementation(
            ...
            mlflow.log_param("base_arch_name", inst.name)
            mlflow.log_param("num_classes", inst.num_classes)
            # mlflow.log_param("resizer_height", inst._image_resizer_fn.keywords['new_height'])
            # mlflow.log_param("resizer_width", inst._image_resizer_fn.keywords['new_width'])
            # mlflow.log_param("feature_extractor", inst.feature_extractor.name)
            # mlflow.log_param("post_processing_iou_thresh", inst._non_max_suppression_fn.keywords['iou_thresh'])
            # mlflow.log_param("post_processing_max_det", inst._non_max_suppression_fn.keywords['max_total_size'])
            # mlflow.log_param("post_processing_max_size_perclass", inst._non_max_suppression_fn.keywords['max_size_per_class'])
            ...
            with batch_metrics_logger(run_id) as metrics_logger:
                ...
                history = original(inst, *args, **kwargs)
                mlflow.log_param("lenght",len(history))
                mlflow.log_param("0",type(history[0]))
                mlflow.log_param("1",type(history[1]))
                mlflow.log_param("his",type(history))


                mlflow.log_metric("TotalLoss", history[0].numpy())
                # log eval metrics
                # c_step = 1
                c_step = history[2].numpy()
                for key, value in history[1].items():
                    ...
                    mlflow.log_metric(metric_name, value.numpy(), step=c_step)
                # lr_name = 'Train/LearningRate'
                # mlflow.log_metric(lr_name, history[2].numpy(), step=c_step)
    ...
    managed = [
        ...
        (object_detection.model_lib_v2, "eager_train_step", EagerTrain),#Added
